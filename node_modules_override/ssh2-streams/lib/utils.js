...
function decryptKey(keyInfo, passphrase) {
  ...
  dc = crypto2.createDecipheriv(keyInfo.encryption, key, iv);
    ...
    if (keyInfo.type === 'rsa') {
      ...
      switch (ecCurveOID) {
        case '1.2.840.10045.3.1.7':
          ...
          tempECDH = crypto2.createECDH('prime256v1');
          break;
        case '1.3.132.0.34':
          // secp384r1
          keyInfo.curve = ecCurveName = 'nistp384';
          tempECDH = crypto2.createECDH('secp384r1');
          break;
        case '1.3.132.0.35':
          // secp521r1
          keyInfo.curve = ecCurveName = 'nistp521';
          tempECDH = crypto2.createECDH('secp521r1');
          break;
        default:
...
function verifyPPKMAC(keyInfo, passphrase, privateKey) {
  ...
  var mackey = crypto2.createHash('sha1')
                     .update('putty-private-key-file-mac-key', 'ascii')
                     .update(passphrase, 'utf8')
                     .digest();

  var calcMAC = crypto2.createHmac('sha1', mackey)
                      .update(macdata)
                      .digest('hex');
