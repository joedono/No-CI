...
var crypto2 = window.crypto2;
var zlib = window.zlib;
var TransformStream = window.stream.Transform;
...
function check_KEXINIT(self, init, firstFollows) {
  ...
  switch (kex_algorithm) {
    case 'diffie-hellman-group1-sha1':
      state.kexdh = 'group';
      state.kex = crypto2.getDiffieHellman('modp2');
      break;
    case 'diffie-hellman-group14-sha1':
      state.kexdh = 'group';
      state.kex = crypto2.getDiffieHellman('modp14');
      break;
    case 'ecdh-sha2-nistp256':
      state.kexdh = 'ec-sha256';
      state.kex = crypto2.createECDH(SSH_TO_OPENSSL[kex_algorithm]);
      break;
...
function onKEXDH_GEX_GROUP(self, prime, gen) {
  ...
  state.kex = crypto2.createDiffieHellman(prime, gen);
  outstate.pubkey = state.kex.generateKeys();
  var idx = 0;
...
function onKEXDH_REPLY(self, info, verifiedHost) { // Client
  ...
  var hash = crypto2.createHash(hashAlgo);
  ...
  var verifier = crypto2.createVerify(keyAlgo);
...
function onNEWKEYS(self) { // Client/Server
  ...
  if (!isStreamCipher(outstate.encrypt.type)) {
    iv = crypto2.createHash(dhHashAlgo)
               .update(secret)
  ...
  key = crypto2.createHash(dhHashAlgo)
              .update(secret)
              .update(outstate.exchangeHash)
  ...
  if (outstate.encrypt.isGCM) {
    ...
    outstate.encrypt.instance = crypto2.createCipheriv(cipherAlgo, key, iv);
  ...
  instate.decrypt.instance = crypto2.createDecipheriv(decipherAlgo, key, iv);
...
function hmacVerify(self, data) {
  ...
  if (instate.decrypt.isGCM) {
    ...
    decrypt.instance = crypto2.createDecipheriv(
    ...
    var calcHmac = crypto2.createHmac(SSH_TO_OPENSSL[hmac.type], hmac.key);
...
function KEXDH_REPLY(self, e) { // Server
  ...
  var signer = crypto2.createSign(signAlgo);
  var signature;
...
function send_(self, payload, cb) {
  ...
  var padBytes = crypto2.randomBytes(padLen);
  padBytes.copy(buf, 5 + payload.length);

  if (hmac.type !== false && hmac.key) {
    mac = crypto2.createHmac(SSH_TO_OPENSSL[hmac.type], hmac.key);
  ...
  if (encrypt.instance !== false) {
    if (encrypt.isGCM) {
      var encrypter = crypto2.createCipheriv(SSH_TO_OPENSSL[encrypt.type],
                                            encrypt.key,
                                            encrypt.iv);
...
function randBytes(n, cb) {
  crypto2.randomBytes(n, function retry(err, buf) {
    if (err)
      return crypto2.randomBytes(n, retry);
    cb && cb(buf);
  });
}
